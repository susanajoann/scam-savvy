...
function encouragement(scorePct: number): string {
  if (scorePct >= 80) {
  ...
  try {
    ...
    const html = `
      <div style="max-width:520px;margin:0 auto;font-family:'Segoe UI',Arial,sans-serif;color:#1A0A3C;">
        ...
        <p style="font-size:14px;color:#7A5FAA;margin:0 0 24px;">${monthLabel(targetMonth)}</p>
        ...
        <p style="font-size:15px;line-height:1.7;color:#333;margin:0 0 24px;">
          ${encouragement(summary.score_pct)}
        </p>
